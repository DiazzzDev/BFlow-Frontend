...
export const LandingFooter = () => {
    ...
    return (
        <footer className="border-t border-border-subtle px-20 pt-12 pb-8">
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-3">
                        {navLinks.map(({ label, id }: { label: string; id: string }) => (
                            <button key={id} onClick={() => handleNavClick(id)} className="text-sm text-text-muted hover:text-white transition-colors text-left cursor-pointer">
                                {label}
                            </button>
                        ))}
                    </div>
            ...
            <div className="flex items-center justify-center border-t border-border-subtle pt-6">
                <p className="text-xs text-text-label">© 2026 BFlow. Todos los derechos reservados.</p>
            </div>
        </footer>
    )
}
