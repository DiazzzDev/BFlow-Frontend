...
import { WalletItem } from "../components/WalletItem.tsx"
import { WalletItemSkeleton } from "../components/WalletItemSkeleton.tsx"
...
const getEmptyDescription = (search: string, activeTab: "wallets" | "sharedWallets") => {
    if (search.trim()) {
...
export const WalletsPage = () => {
    ...
    return (
        <div className="flex gap-6 h-full min-h-0">
                    ...
                    <div className="flex items-center justify-end">
                        <div className="flex items-center gap-1 text-sm">
                            <button
                                ...
                                className={`cursor-pointer transition-colors ${activeTab === "wallets"
                            ...
                            <button
                                ...
                                className={`cursor-pointer transition-colors ${activeTab === "sharedWallets"
                                    ? "text-foreground font-medium"
                                    : "text-muted-foreground hover:text-foreground"
                                    }`}
                            >
            ...
            <aside className="w-full lg:w-80 xl:w-96 shrink-0 border-t lg:border-t-0 lg:border-l border-border flex flex-col px-6 py-5">
                ...
                <ul className="flex flex-col flex-1">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <li
                            key={i}
                            className="min-h-16 border-b border-border last:border-b-0"
                        />
                    ))}
                </ul>
            </aside>
