...
import { useGetWallet, useWalletRouteId } from "./hooks/useGetWallet"
import { WalletDetailSidebar } from "./components/WalletDetailSidebar"
...
export const WalletDetailsPage = () => {
    const walletId = useWalletRouteId()
    const { wallet, isLoading, isNotFound } = useGetWallet(walletId)
    ...
    return (
        <div className="flex gap-0 h-full min-h-0">
            <section className="flex-1 min-w-0 flex flex-col py-5">
                        ...
                        {isLoading ? (
                            ...
                            <>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                    {formatCurrency(wallet?.balance ?? 0, wallet?.currency)} current balance
                                    {" • "}
                                    1 participant
                                </p>
                            </>
                ...
                <div className="flex items-center gap-1 border-b border-border mb-5 px-7">
                    {TABS.map((tab) => (
                        <button
                            key={tab.id}
                            type="button"
                ...
                {activeTab === "overview" ? (
                    <>
                        ...
                        <div className="flex justify-between items-center gap-4 p-5 text-sm text-muted-foreground border-y">
